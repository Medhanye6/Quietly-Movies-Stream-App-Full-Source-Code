...
import {
  View, Text, StyleSheet, Platform, StatusBar as RNStatusBar, Alert, ScrollView
} from "react-native";
...
function MenuItem({ icon, label, sublabel, onPress, danger }: {
  ...
  return (
    <Focusable style={styles.menuItem} onPress={onPress}>
      <View style={[styles.menuIcon, danger && styles.menuIconDanger, { width: scale(44), height: scale(44) }]}>
        <Ionicons name={icon} size={20} color={danger ? Colors.error : Colors.primary} />
      </View>
      <View style={styles.menuText}>
        <Text style={[styles.menuLabel, danger && { color: Colors.error }, { fontSize: sFont(15) }]}>{label}</Text>
        {sublabel && <Text style={[styles.menuSub, { fontSize: sFont(12) }]}>{sublabel}</Text>}
      </View>
...
function StatItem({ label, value }: { label: string; value: string }) {
  return (
    <View style={styles.statBox}>
      <Text style={[styles.statValue, { fontSize: sFont(20) }]}>{value}</Text>
      <Text style={[styles.statLabel, { fontSize: sFont(10) }]}>{label}</Text>
    </View>
  );
}
      ...
      <View style={styles.responsiveWrapper}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
            ...
            <View style={styles.profileInfo}>
              ...
              {user?.isAdmin && (
                <View style={styles.adminBadge}>
                  ...
                  <Text style={[styles.adminTxt, { fontSize: sFont(11) }]}>Premium Member</Text>
                </View>
              )}
            </View>
          ...
          <View style={styles.content}>
            <Text style={[styles.sectionTitle, { fontSize: sFont(11) }]}>{t('accountSettings')}</Text>
            ...
            <Text style={[styles.sectionTitle, { fontSize: sFont(11) }]}>{t('preferences')}</Text>
            <View style={styles.group}>
            ...
            <Text style={[styles.sectionTitle, { fontSize: sFont(11) }]}>{t('supportInfo')}</Text>
            <View style={styles.group}>
            ...
            <View style={styles.footer}>
              <Text style={[styles.appName, { fontSize: sFont(14) }]}>Quietly Stream</Text>
              <Text style={[styles.appVersion, { fontSize: sFont(10) }]}>Version 2.1.0 • Built with Love</Text>
            </View>
          </View>
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  avatarBorder: { borderRadius: 50, padding: 3, alignItems: "center", justifyContent: "center" },
  ...
  userName:     { color: "#fff", fontWeight: "900", marginBottom: 4 },
  userEmail:    { color: "rgba(255,255,255,0.5)", fontWeight: "500" },
  ...
  adminTxt:     { color: Colors.primary, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 },
  ...
  statLabel:    { color: "rgba(255,255,255,0.4)", fontWeight: "600", textTransform: "uppercase", marginTop: 2 },
  statDivider:  { width: 1, height: 24, backgroundColor: "rgba(255,255,255,0.1)" },
  
  content:      { paddingHorizontal: 20, paddingTop: 20 },
  sectionTitle: { color: "rgba(255,255,255,0.4)", fontWeight: "800", textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 12, marginLeft: 8 },
  ...
  menuIcon:     { borderRadius: 14, backgroundColor: "rgba(255,255,255,0.03)", alignItems: "center", justifyContent: "center" },
  menuIconDanger: { backgroundColor: Colors.error + "10" },
  menuText:     { flex: 1 },
  menuLabel:    { color: "#fff", fontWeight: "600" },
  menuSub:      { color: "rgba(255,255,255,0.4)", marginTop: 2 },
  
  footer:       { alignItems: "center", marginTop: 20, opacity: 0.3 },
  appName:      { color: "#fff", fontWeight: "900" },
  appVersion:   { color: "rgba(255,255,255,0.8)", marginTop: 2 },
});
